...
const PERSONAL = {
  ...
  bio: `Specialising in reporting automation, Power BI dashboards, SQL Server ETL pipelines, and operational analytics. Experienced in transforming raw operational data into actionable insights within high precision environments. Passionate about bridging the gap between raw data and strategic decision making`,
...
// ── ADDED: Research publication data ──
const RESEARCH = [
  {
      ...
      <Drawer
        ...
        PaperProps={{
          sx: {
            ...
            borderLeft: "1px solid rgba(0,212,200,0.12)",
          },
        }}
      >
            ...
            <Fade in timeout={900}>
              <Box>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 2.5 }}>
                  <Box sx={{ width: 32, height: 2, bgcolor: "primary.main", borderRadius: 1 }} />
                  <Typography variant="overline" sx={{ color: "primary.main", letterSpacing: "0.22em", fontWeight: 700, fontSize: "0.72rem" }}>
                ...
                <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
                  <Button variant="contained" endIcon={<ArrowForward />} size="large"
                  ...
                  <Button variant="outlined" startIcon={<Download />} size="large"
                    ...
                    sx={{ px: 3, borderColor: "rgba(0,212,200,0.4)", "&:hover": { borderColor: "primary.main", bgcolor: "rgba(0,212,200,0.06)" } }}>
                    Resume
                  </Button>
                  */}
                </Box>
        ...
        <Grid container spacing={6} alignItems="flex-start">
          <Grid item xs={12} md={5}>
            <Typography variant="body1" sx={{ color: "text.secondary", mb: 2.5, lineHeight: 1.95, fontSize: "0.97rem" }}>
              Strategy, Research, Development and Innovation(SRDI) Graduate Engineer at <Box component="span" sx={{ color: "primary.main", fontWeight: 600 }}>ATNS (Air Traffic and Navigation Services)</Box>, where I build end to end analytics solutions from ETL pipelines to executive Power BI dashboards used in safety critical aviation operations.
            </Typography>
            <Typography variant="body1" sx={{ color: "text.secondary", mb: 2.5, lineHeight: 1.95, fontSize: "0.97rem" }}>
           I work closely with stakeholders to turn business requirements into reliable, automated reporting solutions. I'm also actively expanding into Python-based machine learning and predictive analytics, bridging the gap between traditional business intelligence and modern data science.
            ...
            {/* ── ADDED: software background paragraph ── */}
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {[
                ...
                <Box key={label} sx={{ display: "flex", gap: 1.5 }}>
                  ...
                  <Box>
                    <Typography sx={{ fontSize: "0.72rem", fontWeight: 700, color: "primary.main", textTransform: "uppercase", letterSpacing: "0.1em", mb: 0.2 }}>{label}</Typography>
                    <Typography sx={{ fontSize: "0.88rem", color: "text.secondary", lineHeight: 1.7 }}>{value}</Typography>
                  </Box>
                </Box>
              ))}
            </Box>
        ...
        <LinearProgress
          ...
          sx={{
            ...
            "& .MuiLinearProgress-bar": {
              ...
              background:
                level >= 88
                  ? "linear-gradient(90deg,#00d4c8,#5ffff2)"
                  : level >= 75
...
function ProjectCard({ project, onOpen }) {
  return (
    <Card onClick={() => onOpen(project)} sx={{
      ...
      <CardContent sx={{ p: 3.5, flex: 1 }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 2.5 }}>
          ...
          <Chip label={project.tag} size="small" sx={{ bgcolor: `${project.color}10`, border: `1px solid ${project.color}25`, color: project.color, fontWeight: 600, fontSize: "0.65rem" }} />
        </Box>
      ...
      <CardActions sx={{ px: 3.5, pb: 3 }}>
        <Button endIcon={<ArrowForward />} sx={{ color: "primary.main", p: 0, "&:hover": { bgcolor: "transparent", opacity: 0.75 } }}>
          View Details
        </Button>
      </CardActions>
    </Card>
...
function ProjectsSection({ sectionRef }) {
  ...
  const CARD_WIDTH = 376; // 360 + 16 gap
  const GAP = 24;
  ...
  const scroll = (dir) => {
    trackRef.current?.scrollBy({ left: dir === "left" ? -(CARD_WIDTH + GAP) : (CARD_WIDTH + GAP), behavior: "smooth" });
  };
  ...
  return (
    <Box ref={sectionRef} id="projects" component="section" sx={{ py: { xs: 8, md: 12 }, overflow: "hidden" }}>
      ...
      {/* Carousel outer — full width for edge-bleed effect */}
      <Box sx={{ position: "relative" }}>
        {/* Left fade */}
        <Box sx={{
        ...
        <Container maxWidth="lg">
          <Box sx={{ display: "flex", gap: 1.5, justifyContent: "flex-end", mt: 2 }}>
            <IconButton onClick={() => scroll("left")} disabled={!canScrollLeft} sx={{
            ...
            <IconButton onClick={() => scroll("right")} disabled={!canScrollRight} sx={{
              border: "1px solid rgba(0,212,200,0.2)", bgcolor: "background.paper", width: 44, height: 44,
              "&:hover": { bgcolor: "rgba(0,212,200,0.08)", borderColor: "primary.main" },
              "&.Mui-disabled": { opacity: 0.3 },
            }}>
...
function ProjectDialog({ project, onClose }) {
  ...
  return (
    <Dialog open={!!project} onClose={onClose} maxWidth="lg" fullWidth scroll="paper"
      ...
      <DialogTitle sx={{ pt: 3, pb: 2 }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <IconButton onClick={onClose} sx={{ "&:hover": { bgcolor: "rgba(0,212,200,0.08)" } }}><Close /></IconButton>
        </Box>
      </DialogTitle>
      ...
      <DialogContent dividers sx={{ borderColor: "rgba(0,212,200,0.1)" }}>
        ...
        {/* Only render screenshot carousel if there are screenshots */}
              ...
              <Box ref={scrollRef} sx={{
                ...
                {project.screenshots.map((img, index) => (
                  <Box key={index} sx={{
                    ...
                    bgcolor: "rgba(0,212,200,0.03)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    transition: "0.3s ease",
                    "&:hover": { transform: "translateY(-3px)", borderColor: "primary.main" },
                  }}>
                    <Box component="img" src={img} alt={`Screenshot ${index + 1}`}
  ...
  const domains = [
    {
      ...
      color: "#00d4c8",
      items: ["SQL Server (T-SQL, stored procs, views, indexes)", "SSIS packages & ETL pipeline automation", "Data modelling (star schema / snowflake)", "Azure Data Factory (foundational)"],
    },
    {
      title: "Business Intelligence",
      icon: <BarChart sx={{ fontSize: 26 }} />,
      color: "#f59e0b",
      items: ["Power BI (Desktop, Report, Service)", "DAX calculated measures, KPIs & time intelligence", "Power Query (M language transformations)", "Executive, operational & compliance dashboards"],
    },
    {
      title: "Power Platform",
      icon: <Dashboard sx={{ fontSize: 26 }} />,
      color: "#a78bfa",
      items: ["Power Automate – end-to-end workflow automation", "Power Apps – data-driven form and app development", "SharePoint list integration & document management", "Excel-to-SharePoint-to-report automated pipelines"],
    },
    {
      title: "Analytics & Reporting",
      icon: <TrendingUp sx={{ fontSize: 26 }} />,
      color: "#34d399",
      items: ["Operational KPI frameworks & performance monitoring", "Root cause analysis & variance reporting", "Stakeholder engagement – requirements gathering & change management", "User manuals, training materials & presentation delivery"],
    },
    {
      title: "ML & Python",
      icon: <Psychology sx={{ fontSize: 26 }} />,
      color: "#f87171",
      items: ["Python (Pandas, NumPy, Matplotlib, Seaborn)", "Scikit-learn (classification, regression, clustering)", "XGBoost & ensemble methods", "Association Rule Mining (Apriori, FP-Growth) — Honours Research", "TensorFlow & deep learning (currently learning – Udemy)"],
    },
    // ── ADDED: Software Engineering domain ──
    {
      title: "Software Engineering",
      icon: <Code sx={{ fontSize: 26 }} />,
      color: "#fb923c",
      items: ["React.js – component-driven UI development", "Node.js & Express – RESTful API design & development", "PostgreSQL – relational database design & querying", "JavaScript (ES6+) – frontend & backend scripting", "Version control with Git & GitHub"],
    },
    {
      title: "Soft Skills & Domain",
      icon: <Handshake sx={{ fontSize: 26 }} />,
      color: "#60a5fa",
      items: ["Stakeholder & client engagement", "Safety-critical data environments (aviation)", "Technical documentation & process documentation", "Cross-functional collaboration with management"],
    },
  ];
              ...
              <Card sx={{ height: "100%", p: 0.5 }}>
                <CardContent sx={{ p: 3 }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 3 }}>
                    <Box sx={{ width: 50, height: 50, borderRadius: 2, bgcolor: `${color}10`, border: `1px solid ${color}20`, display: "flex", alignItems: "center", justifyContent: "center", color, flexShrink: 0 }}>
                      {icon}
                    </Box>
                    <Typography variant="h5" sx={{ color: "text.primary", fontFamily: "'Playfair Display', serif", lineHeight: 1.25 }}>{title}</Typography>
                  </Box>
                  <Box sx={{ display: "flex", flexDirection: "column", gap: 1.2 }}>
                    {items.map((item) => (
                      <Box key={item} sx={{ display: "flex", alignItems: "flex-start", gap: 1.5 }}>
                        ...
                        <Typography variant="body2" sx={{ color: "text.secondary", lineHeight: 1.7, fontSize: "0.86rem" }}>{item}</Typography>
                      </Box>
                    ))}
                  </Box>
                </CardContent>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {EDUCATION.map((edu) => (
                <Box key={edu.degree} sx={{
                  ...
                  <Box sx={{ width: 42, height: 42, borderRadius: 2, bgcolor: `${edu.color}10`, border: `1px solid ${edu.color}25`, display: "flex", alignItems: "center", justifyContent: "center", color: edu.color, flexShrink: 0 }}>
                    {edu.icon}
                  </Box>
                  <Box>
                    <Typography sx={{ fontWeight: 600, color: "text.primary", fontSize: "0.95rem", lineHeight: 1.4 }}>{edu.degree}</Typography>
                    <Typography sx={{ color: "text.secondary", fontSize: "0.8rem", mt: 0.3 }}>Mathematical Sciences & Computer Science</Typography>
                  </Box>
                </Box>
              ))}
            </Box>
          ...
          <Grid item xs={12} md={7}>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {CERTIFICATIONS.map((cert) => (
                <Box key={cert.name} sx={{
                  p: 3, borderRadius: 3, bgcolor: "background.paper",
                  border: "1px solid rgba(0,212,200,0.1)",
                  display: "flex", gap: 2, alignItems: "flex-start",
                  transition: "0.3s", "&:hover": { borderColor: "rgba(0,212,200,0.3)" },
                }}>
                  <Box sx={{ width: 42, height: 42, borderRadius: 2, bgcolor: `${cert.color}10`, border: `1px solid ${cert.color}25`, display: "flex", alignItems: "center", justifyContent: "center", color: cert.color, flexShrink: 0 }}>
                    {cert.icon}
                  </Box>
                  <Box sx={{ flex: 1 }}>
                    <Typography sx={{ fontWeight: 600, color: "text.primary", fontSize: "0.92rem", lineHeight: 1.4 }}>{cert.name}</Typography>
                    <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mt: 0.5 }}>
                      <Typography sx={{ color: "text.secondary", fontSize: "0.8rem" }}>{cert.issuer}</Typography>
                      <Chip label={cert.status} size="small" sx={{
                        bgcolor: cert.status === "Completed" ? "rgba(52,211,153,0.1)"
                          : cert.status === "Enrolled"     ? "rgba(0,212,200,0.1)"
                          : "rgba(122,155,191,0.1)",
        ...
        {RESEARCH.map((paper) => (
          <Card
            ...
            sx={{
              ...
              "&:hover": { border: `1px solid ${paper.color}45` },
            }}
          >
    ...
    <Box ref={sectionRef} id="contact" sx={{ py: { xs: 8, md: 12 }, bgcolor: "rgba(7,24,41,0.7)", borderTop: "1px solid rgba(0,212,200,0.06)" }}>
      <Container maxWidth="md">
        ...
        <Typography sx={{ textAlign: "center", color: "text.secondary", lineHeight: 1.95, mb: 5, fontSize: "0.97rem" }}>
          Open to <Box component="span" sx={{ color: "primary.main", fontWeight: 600 }}>Data Analyst, BI Engineer, Reporting Analyst, Business Intelligence</Box>, and <Box component="span" sx={{ color: "primary.main", fontWeight: 600 }} >Analytics Engineering</Box> opportunities. Feel free to reach out for collaborations, project inquiries, or just a chat about data and analytics!
        </Typography>
        ...
        <Box sx={{ display: "flex", gap: 2.5, justifyContent: "center", flexWrap: "wrap", mb: 5 }}>
          {[
            ...
            <Box key={label} component="a" href={href} sx={{
              ...
              <Box>
                <Typography sx={{ fontSize: "0.68rem", color: "text.secondary", textTransform: "uppercase", letterSpacing: "0.08em" }}>{label}</Typography>
                <Typography sx={{ fontSize: "0.88rem", color: "text.primary", fontWeight: 500 }}>{value}</Typography>
              </Box>
            </Box>
          ))}
        </Box>
{/*
     <Box sx={{ display: "flex", justifyContent: "center", gap: 2, flexWrap: "wrap" }}>
          <Button variant="contained" startIcon={<Email />} href={`mailto:${PERSONAL.email}`}
            sx={{ px: 3.5, background: "linear-gradient(135deg,#00d4c8,#009e94)" }}>
            Email Me
          </Button>
          <Button variant="outlined" startIcon={<LinkedIn />} href={PERSONAL.linkedin} target="_blank"
            sx={{ px: 3, borderColor: "rgba(0,212,200,0.35)" }}>
            LinkedIn
          </Button>
          <Button variant="outlined" startIcon={<GitHub />} href={PERSONAL.github} target="_blank"
            sx={{ px: 3, borderColor: "rgba(0,212,200,0.35)" }}>
            GitHub
          </Button>
        </Box>
...
function App() {
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <ResearchSection sectionRef={sections.Research} />  {/* ← added */}
